...
int main(int argc, char *argv[]) {
    ...
    memset(to, 'y', 100000);
    check(valid_copy(to, 100000, 'y'), "Not initalized right.");

    clock_t start = clock();
    // use normal copy to
    rc = normal_copy(from, to, 100000);
    clock_t end = clock();
    printf("time for normal copy: %ld\n", end-start);
    ...
    printf("time for duffs device: %ld\n", end-start);
    check(rc == 100000, "Duffs device failed: %d", rc);
    check(valid_copy(to, 100000, 'x'), "Duffs device failed.");
    ...
    printf("time for zeds device: %ld\n", end-start);
    check(rc == 100000, "Zeds device failed: %d", rc);
    check(valid_copy(to, 100000, 'x'), "Zeds device failed.");
