...
static inline void radix_sort(short offset, uint64_t max, uint64_t *source, uint64_t *dest) {
    ...
    // transfer count into index by summing elements and sorting into same array
    ...
    // fill dest with the right values in the right places
    for (sp=source, end=source+256; sp<end; sp++) {
...
int RadixMap_delete(RadixMap *map, RMElement *el) {
    check(map->end > 0, "There's is nothing to delete.");
